module.exports = function init(site) {
  ...
  site.post("/api/amounts_in/add", (req, res) => {
    ...
    $amounts_in.add(amounts_in_doc, (err, doc) => {
      if (!err) {
        site.call('[amount in] please add money to safe' , doc  )
        site.call('[amount in] please add money to safe payments' ,  Object.assign({} ,doc ))
        response.done = true
      } else {
  ...
  site.post("/api/amounts_in/delete", (req, res) => {
    ...
    if (_id) {
      $amounts_in.delete({
        ...
        if (!err) {
          site.call('[amount in] please discount money from safe' , result.doc)
          site.call('[amount in] please delete row from safe payments' ,  Object.assign({} ,result.doc ))
          response.done = true
        }
        res.json(response)
      })
