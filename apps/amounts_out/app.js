  ...
  site.post("/api/amounts_out/add", (req, res) => {
    ...
    $amounts_out.add(amounts_out_doc, (err, doc) => {
      if (!err) {
        ...
        site.call('[amount out] please discount money from safe' , doc)
        site.call('[amount out] please add money to safe payments' ,  Object.assign({} ,doc ))
        response.done = true
      }else{
  ...
  site.post("/api/amounts_out/delete", (req, res) => {
    ...
    if (_id) {
      $amounts_out.delete({ _id: $amounts_out.ObjectID(_id), $req: req, $res: res }, (err, result) => {
        if (!err) {
          site.call('[amount out] please add money to safe' , result.doc  )
          site.call('[amount out] please delete row from safe payments' ,  Object.assign({} ,result.doc ))
          response.done = true
        }
        res.json(response)
      })
