module.exports = function init(site) {
  ...
  site.post("/api/employee_discount/add", (req, res) => {
    ...
    $employee_discount.add(employee_discount_doc, (err, doc) => {
      if (!err) {
        site.call('[employee discount] please add money to safe' ,  Object.assign({} ,doc ))
        site.call('[employee discount] please add money to safe payments' ,  Object.assign({} ,doc ))
  ...
  site.post("/api/employee_discount/delete", (req, res) => {
    ...
    if (_id) {
      $employee_discount.delete({
        ...
        if (!err) {
          site.call('[employee discount] please discount money from safe' , result.doc)

          site.call('[employee discount] please delete row from safe payments' , result.doc)
          response.done = true
        }
        res.json(response)
      })
